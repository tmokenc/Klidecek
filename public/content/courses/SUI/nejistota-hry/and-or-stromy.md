...
V *nedeterministických* prostředích už nestačí najít *sekvenci akcí* — musíme najít **strategii / plán** (contingency plan): co dělat *v každé možné situaci*, do které se můžeme dostat.
...
::: viz and-or-tree-plan "AND-OR strom pro nondeterministický vysavač; podmíněný plán s cyklem; AND/OR vrstvy barevně odlišené."
:::
...
`if` test musí umět agent vyhodnotit *za běhu* — vyžaduje *senzory*. Bez senzorů máme *senseless* prostředí (viz [[castecna-pozorovatelnost]]).
...
* **MDP** (Markov Decision Processes) — stochastické nedeterministické akce s pravděpodobnostmi. Řešení = *policy* (pravidlo akce na každý stav).
* **POMDP** (Partially Observable MDP) — agent navíc nevidí přesný stav. Řešení = policy nad *belief stavem* (distribucí stavů).

Tyto formalizmy jsou centrální pro [[rl-framework]] (reinforcement learning).
