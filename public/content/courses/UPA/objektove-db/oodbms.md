...
* **S**patio-temporal priority (časoprostorová priorita) — dva objekty reprezentující *tutéž* entitu reálného světa musí mít *jediný* OID v rámci celé aplikace, a tedy být identické (na rozdíl od dvou různých entit se shodným stavem z úvodu kapitoly, které mohou být jen ekvivalentní). Příklad: dvě načtení téhož zákazníka — instance `Customer` se stejným `ID` ze dvou různých modulů aplikace — musí vést na jeden a tentýž objekt.
