...
## Relace happens-before

**Leslie Lamport** (1978) definoval relaci **happens-before** (česky „stalo se před") $\to$ na událostech v distribuovaném systému. Tato relace zachycuje, která událost mohla kauzálně ovlivnit kterou:
...
Relace happens-before je **parciální** (částečné) uspořádání — neuspořádá každou dvojici událostí. Pro **úplné** uspořádání rozšíříme čas o **ID procesu**:

::: math
C^+(a) = (C(a),\ \text{ID}(\text{process of } a))
:::

S lexikografickým uspořádáním. Díky tomu jsou pak *všechny* události navzájem *uspořádány*. Toto se používá například v *Lamportově algoritmu vzájemného vyloučení* (viz [[vzajemne-vylouceni]]).
...
**Pravidlo 2** (odeslání zprávy): pošle *celý vektor* se zprávou:
...
Po obdržení zprávy od p1 ve stavu (2,0): p2 nastaví hodnoty na $(\max(0,2), \max(1,0)) = (2,1)$ a zvýší svou složku $\to (2,2)$.
...
- **Distribuované databáze** — verzovací vektory (version vectors) v systémech Riak a DynamoDB pro detekci konfliktních zápisů.
- **Kauzální rozesílání (causal broadcast)** — rozesílání zpráv, které zachovává kauzální uspořádání.
- **Bezkonfliktní replikované datové typy (conflict-free replicated data types, CRDTs)** — synchronizace replik bez nutnosti koordinace.
- **Ladění (debugging) distribuovaných aplikací** — sledování *kauzálních* závislostí mezi událostmi.

## Limity vektorových hodin

- **Velikost vektoru = $n$** (počet procesů). Ve *velkých* systémech to může být problematické.
- **Optimalizace**: maticové hodiny (*matrix clocks*) sledují, co který proces ví o vědomostech ostatních — jsou ještě silnější, ale zabírají ještě více místa.
- **Věrohodné hodiny (plausible clocks)** — heuristické aproximace s menší přesností.

## Lamportovy vs. vektorové hodiny — kdy co použít

| Kritérium | Lamportovy | Vektorové |
| :--- | :---: | :---: |
| Velikost razítka | $O(\log T)$ | $O(n \log T)$ |
| Kauzální shoda | jen $\Rightarrow$ | plně $\iff$ |
| Detekce paralelnosti | ne | ano |
| Použití | vzájemné vyloučení, rozesílání | CRDT, ladění |

**Lamportovy hodiny** stačí pro *uspořádání*. **Vektorové hodiny** jsou nutné pro *detekci paralelnosti*.

## Co dál

[[volba-master]] probere algoritmy pro **volbu hlavního uzlu** (leader election) — *fundamentální* stavební blok pro distribuovaný konsensus. Mezi klasické patří: **Chang-Roberts** (jednoduchý, O(n²)), **Hirschberg-Sinclair** (O(n log n) na kruhu). [[vzajemne-vylouceni]] potom probere algoritmy pro **distribuované vzájemné vyloučení** (mutual exclusion, mutex) — *Lamportův* algoritmus (postavený přímo na logických hodinách!), Ricart-Agrawala, Maekawa, Raymond.
