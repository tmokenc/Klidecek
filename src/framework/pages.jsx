...
function scrollMainTo(options) {
  const el = document.querySelector("main.content");
  if (el && typeof el.scrollTo === "function") el.scrollTo(options);
...
export function VerifiedBadge({
  size = 14,
  title = "Verified — reviewed by a human",
}) {
  return (
    <span
...
export function SpecChip({ content, id }) {
  ...
  if (!s) return null;
  return (
    <span className="chip spec" style={{ "--h": s.hue }}>
      {s.id}
    </span>
  );
}
...
function UnavailPill({ label = "not yet available" }) {
  return (
    <span
      className="unavail-pill"
      title="This exam-topic content hasn't been authored yet."
    >
      {label}
    </span>
  );
  ...
  return (
    <div className="ai-notice" role="note">
      ...
      <div className="ai-notice-body">
        <strong>Most of this content is AI-generated</strong> and built on top
        of <strong>one student's personal lecture notes</strong>. Two
        consequences: some material may be <em>missing</em> (topics the lecturer
        skipped or the notes don't cover) and some may be{" "}
        <em>over-detailed or off-syllabus</em> (kept because it was on a saved
        slide). The official VUT FIT okruhy PDF is the authoritative scope.
        Items marked with a blue <VerifiedBadge size={12} title="" /> have been
        reviewed by a human; treat everything else as a study aid and
        cross-check against the course slides, RFCs, or textbooks.{" "}
        {repoUrl && (
          <>
            Spotted something wrong, have an improvement, or verified a topic
            yourself?{" "}
            <a
...
function useAiSessionToast() {
  useEffect(() => {
    try {
      if (sessionStorage.getItem("okruhy.aiToastShown")) return;
    } catch {
      return;
    }
    ...
    const id = setTimeout(() => {
      try {
        sessionStorage.setItem("okruhy.aiToastShown", "1");
      } catch {}
      if (window.toast)
        window.toast(
          "AI-generated unless marked ✓ — verify with primary sources or contribute on GitHub",
          6500,
        );
    }, 600);
...
export function HomePage({ content, navigate }) {
  ...
  const tot = content.SPECIALIZATIONS.reduce(
    (s, sp) => s + specStats(sp.id).done,
    0,
  );
  return (
    <>
      <div className="page-head" style={{ marginTop: 24 }}>
        ...
        <h1 className="page-title" style={{ fontSize: 34, marginBottom: 12 }}>
          Three ways to study.
        </h1>
        <p className="page-blurb">
          Browse by course, by your specialization, or straight from the
          final-exam topic list. Progress lives in your browser — export it as
          JSON to keep it.
        </p>
      </div>
      ...
      <div className="cards" style={{ marginTop: 12 }}>
        ...
        >
          <div className="card-top">
            ...
            >
              {content.COURSES.length} courses
            </span>
          </div>
          <p className="card-blurb">
            All material organized by individual course → topic → subtopic.
          </p>
        </button>
        ...
        >
          <div className="card-top">
            ...
            >
              {content.SPECIALIZATIONS.length} tracks
            </span>
          </div>
          <p className="card-blurb">
            Filter to your master's track — see only its courses and its
            final-exam topics.
          </p>
        </button>
        ...
        >
          ...
          <p className="card-blurb">
            Drill straight into state-exam topic areas. Shared areas across
            specializations are color-badged.
          </p>
        </button>
      </div>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: "var(--text-muted)" }}>
              See how all {content.COURSES.length} courses connect — shared
              concepts, prerequisites, and crossover topics.
            </p>
          </div>
          <svg
            width="14"
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h3 className="card-title" style={{ marginBottom: 2 }}>
            Komise — co se ptají u státnic
          </h3>
          <p style={{ margin: 0, fontSize: 13, color: "var(--text-muted)" }}>
            Zadej, kdo tě bude zkoušet, a uvidíš, na co se daní komisaři ptali —
            namapováno na okruhy. Data z externích repozitářů.
          </p>
        </div>
        <svg
          width="14"
      ...
      {tot > 0 && (
        ...
        >
          You've checked off{" "}
          <strong style={{ color: "var(--text)" }}>{tot}</strong> subtopics.
          Keep going.
        </div>
      )}
    </>
...
export function GlobalMindmapPage({ content, navigate }) {
  ...
  if (!data)
    return (
      <div className="empty">
        Global mindmap not available — content/mindmaps/_global.json missing.
      </div>
    );
  return (
    <>
  ...
  return (
    <>
      <div className="page-head">
        ...
        <p className="page-blurb">
          Every course you can study. Drill in to see topics and subtopics, jump
          to specific material via deep-linkable anchors, or open the mindmap.
        </p>
        {content.GLOBAL_MINDMAP && (
          <button
            className="btn"
          ...
          return (
            ...
            >
              ...
              <div className="card-meta">
                <span>
                  {c.topics.length} topics · {subCount} subtopics
                </span>
                <span style={{ flex: 1 }} />
                {st.total > 0 && (
                  <>
    ...
    >
      <div className="toc-sidebar-head">
        ...
        <button
          ...
          onClick={onToggle}
          aria-label={
            hidden ? "Show table of contents" : "Hide table of contents"
          }
          title={hidden ? "Show TOC" : "Hide TOC"}
        >
          <svg
            width="13"
            height="13"
        ...
        {course.topics.map((t, i) => {
          ...
          return (
            <div key={t.id} className="toc-side-topic" data-active={isActive}>
              <button
                className="toc-side-topic-btn"
                onClick={() => onJumpTopic(t.id)}
              >
                <span className="toc-side-num">
              ...
              <div className="toc-side-subs">
                {t.subtopics.map((s) => {
                  ...
                  return (
                    <button
                      key={s.id}
                      className="toc-side-sub"
                      data-active={activeSub === key}
                      data-sub-key={key}
...
export function CourseDetailPage({
  ...
  const [activeTopic, setActiveTopic] = useState(
    focusTopic || (course && course.topics[0] && course.topics[0].id),
  );
  ...
  useEffect(() => {
    ...
    if (focusTopic && focusSub)
      setCollapsed(`sub:${course.id}/${focusTopic}/${focusSub}`, false);
        ...
        requestAnimationFrame(() => {
          ...
          const figEl =
            ...
            document.getElementById(
              `sub-${focusTopic}-${focusSub}--${focusFig}`,
            );
          const el =
            figEl || document.getElementById(`sub-${focusTopic}-${focusSub}`);
          if (el) {
            el.scrollIntoView({
              behavior: "smooth",
              block: figEl ? "center" : "start",
            });
  ...
  useEffect(() => {
    if (!course || isMindmap) return;
    const els = course.topics
      .map((t) => document.getElementById(`top-${t.id}`))
      .filter(Boolean);
    ...
    return () =>
      topicObserverRef.current && topicObserverRef.current.disconnect();
  ...
  useEffect(() => {
    ...
    const offset =
      pillRect.left + pillRect.width / 2 - (barRect.left + barRect.width / 2);
    ...
    <>
      <div className="detail-head">
        ...
        <div className="detail-meta">
          {(course.specializations || []).map((id) => (
            <SpecChip key={id} content={content} id={id} />
          ))}
          <span style={{ flex: 1 }} />
          {st.total > 0 && (
            <div className="hist">
      ...
      {isMindmap ? (
        <Mindmap
          ...
          completedSet={set}
          content={content}
          onNavigate={(cid, tid, sid) =>
            navigate(`/c/${cid}/${tid}${sid ? "/" + sid : ""}`)
          }
        />
      ) : (
        <>
          ...
          />
          <TocSidebarReopen
            onToggle={() => setSidebarHidden(false)}
            hidden={sidebarHidden}
          />
          ...
          {course.topics.map((t, ti) => {
            ...
            return (
              <section
                ...
                id={`top-${t.id}`}
                key={t.id}
                data-collapsed={topicCollapsed}
              >
                <h2 className="topic-title">
                  <ChevronToggle
                    collapsed={topicCollapsed}
                    onClick={() => toggleCollapsed(topicKey)}
                    label={t.title}
                  />
                ...
                {!topicCollapsed &&
                  t.subtopics.map((sub) => {
                    ...
                    const subTier =
                      sub.tier && !sub.tier.core ? sub.tier : null;
                    const subDefaultCollapsed = !!(
                      subTier && !subTier.defaultOpen
                    );
                    const subCollapsed = isCollapsed(
                      subKey,
                      subDefaultCollapsed,
                    );
                    return (
                      <article
                        ...
                        data-tier={subTier ? subTier.kind : undefined}
                        style={
                          subTier ? { "--tier-h": subTier.hue } : undefined
                        }
                      >
                        <header className="subtopic-head">
                          <div className="subtopic-title-row">
                            <ChevronToggle
                              ...
                              onClick={() =>
                                toggleCollapsed(subKey, subDefaultCollapsed)
                              }
                              label={sub.title}
                            />
                            <button
                          ...
                          <button
                            className={
                              "subtopic-anchor" + (isCopied ? " copied" : "")
                            }
                        ...
                        {!subCollapsed && (
                          <LazyBlocks
                            ...
                            subId={sub.id}
                            forceMount={
                              focusTopic === t.id && focusSub === sub.id
                            }
                          />
                        )}
                      </article>
                  ...
                  <li key={i}>
                    ...
                    >
                      <div className="see-more-link-text">
                        ...
                        {r.note && (
                          <span className="see-more-link-note">{r.note}</span>
                        )}
                      </div>
                      {r.kind && (
                        <span className="see-more-kind">{r.kind}</span>
                      )}
                      <svg
                        width="12"
                        height="12"
  ...
  return (
    <>
      <div className="page-head">
        ...
        <p className="page-blurb">
          Filter material by your master's specialization. Each track has its
          own course set and final-exam topic areas.
        </p>
      </div>
      <div className="cards">
        {content.SPECIALIZATIONS.map((s) => {
          const courseCount = content.COURSES.filter((c) =>
            (c.specializations || []).includes(s.id),
          ).length;
          const topics = content.EXAM_TOPICS[s.id] || [];
          const examCount = topics.length;
          const coveredCount = topics.filter((t) =>
            topicHasContent(content, t),
          ).length;
          ...
          return (
            ...
            >
              <div className="card-top">
                ...
                <span
                  style={{
                    ...
                    height: 28,
                    borderRadius: "50%",
                    background: `oklch(0.62 0.14 ${s.hue})`,
                    flexShrink: 0,
                    opacity: blank ? 0.5 : 1,
                  }}
                />
              ...
              <div className="card-meta">
                <span>
                  {courseCount} courses · {examCount} exam topics
                  {examCount > 0 && coveredCount < examCount
                    ? ` (${coveredCount} with content)`
                    : ""}
                </span>
                <span style={{ flex: 1 }} />
                {blank && <UnavailPill />}
...
export function SpecDetailPage({ content, specId, navigate }) {
  ...
  const courses = content.COURSES.filter((c) =>
    (c.specializations || []).includes(specId),
  );
  const examTopics = content.EXAM_TOPICS[specId] || [];
  const coveredCount = examTopics.filter((t) =>
    topicHasContent(content, t),
  ).length;
    ...
    <>
      ...
      {specBlank && (
        <div className="unavail-banner">
          <strong>Coming soon.</strong> This specialization's exam topics are
          listed below for reference, but study material hasn't been authored
          yet. Shared topics that overlap with other specs may already have
          content — open one to find out.
        </div>
      )}
      ...
      <div className="exam-topiclist">
        {examTopics.map((t) => {
          ...
          return (
            <button
              key={t.id}
              className="exam-topic-card"
              data-blank={blank}
              onClick={() => navigate(`/x/${specId}/${t.id}`)}
            >
              <span className="exam-topic-n">
              ...
              {blank &&
                (hasLinks ? (
                  <span
                    className="exam-links-pill"
                    title={`${t.resources.length} external resources`}
                  >
                    {t.resources.length} link
                    {t.resources.length === 1 ? "" : "s"}
                  </span>
                ) : (
              ...
              <span className="exam-topic-tags">
                {t.sharedWith && t.sharedWith.length > 0 && (
                  <SpecDots content={content} specs={t.sharedWith} />
                )}
              </span>
            </button>
          );
      ...
      {ex.total > 0 && (
        <div className="hist" style={{ marginTop: 12 }}>
          <span style={{ fontFamily: "var(--font-mono)" }}>
            {ex.done}/{ex.total} sub-areas covered
          </span>
      ...
      >
        <span
          style={{
            ...
            height: 10,
            borderRadius: "50%",
            background: `oklch(0.62 0.14 ${spec.hue})`,
          ...
          {content.SPECIALIZATIONS.map((s) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ flex: 1, fontWeight: active ? 600 : 500 }}>
                  {s.name}
                </span>
                <span
                  style={{
  ...
  return (
    <>
      <div className="page-head">
        ...
        <p className="page-blurb">
          State-exam topic areas are grouped by specialization. Some areas are
          shared across multiple specializations — marked with colored badges.
        </p>
      </div>
      <div className="cards">
        {content.SPECIALIZATIONS.map((s) => {
          const topics = content.EXAM_TOPICS[s.id] || [];
          const sharedCount = topics.filter(
            (t) => t.sharedWith && t.sharedWith.length,
          ).length;
          const coveredCount = topics.filter((t) =>
            topicHasContent(content, t),
          ).length;
...
function ExamTopicCard({ content, specId, t, set, navigate }) {
  ...
  return (
    <div
      className="exam-topic-card"
      data-blank={blank}
      data-has-speedrun={!!sr}
    >
      <button
        className="exam-topic-row"
        onClick={() => navigate(`/x/${specId}/${t.id}`)}
      >
        ...
        {blank ? (
          hasLinks ? (
            <span
              ...
              title={`${t.resources.length} external resources`}
            >
              {t.resources.length} link{t.resources.length === 1 ? "" : "s"}
            </span>
          ) : (
            <UnavailPill label="empty" />
          )
      ...
      {sr && (
        <>
          <button
            className="exam-speedrun-toggle"
            data-open={open}
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
          >
            ...
            {sr.grounded && (
              <span
                className="exam-speedrun-tag"
                title="Sestaveno z dřívějších dotazů u komise"
              >
                dle komise
              </span>
            )}
            <span className="exam-speedrun-caret">
              {open ? "▾ skrýt" : "▸ rozbalit"}
            </span>
          </button>
          {open && (
  ...
  return (
    <>
      ...
      <div className="spec-hero" style={{ "--h": spec.hue, marginTop: 14 }}>
        ...
        <p className="page-blurb">
          {topics.length} topic areas. Open any to study. Use ← → on desktop or
          the buttons on mobile to move between areas.
        </p>
        <div className="detail-meta" style={{ marginTop: 14 }}>
          {st.total > 0 && (
            <div className="hist">
              <span style={{ fontFamily: "var(--font-mono)" }}>
                {st.done}/{st.total} sub-areas covered
              </span>
              <ProgressBar pct={st.pct} />
            </div>
          )}
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "ArrowLeft" && idx > 0)
        navigate(`/x/${specId}/${topics[idx - 1].id}`);
      if (e.key === "ArrowRight" && idx < topics.length - 1)
        navigate(`/x/${specId}/${topics[idx + 1].id}`);
    };
    ...
    <>
      ...
      >
        ...
        <button
          ...
          onClick={() => navigate(`/x/${specId}`)}
          title="All topics in this specialization"
        >
          <svg
            width="13"
            height="13"
      ...
      <div className="exam-nav">
        <button
          className="exam-nav-prev"
          disabled={idx === 0}
          onClick={() =>
            idx > 0 && navigate(`/x/${specId}/${topics[idx - 1].id}`)
          }
        >
          <svg
            width="15"
        ...
        <div className="exam-nav-info">
          <span className="en-eyebrow">
            {spec.id} · topic {topic.n}
          </span>
          <span className="en-title">{topic.title}</span>
        </div>
        <button
          className="exam-nav-next"
        ...
        >
          ...
          {topic.sharedWith.map((sid) => (
            <button
              ...
              className="chip spec"
              style={{
                "--h": (content.findSpec(sid) || {}).hue || 264,
                cursor: "default",
              }}
              onClick={() => {
                const otherTopics = content.EXAM_TOPICS[sid] || [];
                const match =
                  otherTopics.find((o) => o.title === topic.title) ||
                  otherTopics.find((o) =>
                    o.refs.some((r) =>
                      topic.refs.some(
                        (tr) =>
                          tr[0] === r[0] && tr[1] === r[1] && tr[2] === r[2],
                      ),
                    ),
                  );
                if (match) navigate(`/x/${sid}/${match.id}`);
                else navigate(`/x/${sid}`);
              }}
            >
              {sid}
      ...
      {(() => {
        ...
        return (
          <div className="unavail-banner">
            <strong>
              {hasResources
                ? "No in-app study material yet"
                : "Content not yet available."}
            </strong>{" "}
            {hasResources ? (
              <>
                — this exam topic is in the PDF but hasn't been authored in the
                app. Curated external references are listed below to get you
                started.
              </>
            ) : (
              <>
                This exam topic is listed in the official PDF (
                <em>
                  {spec.id} — okruh č.&nbsp;{topic.n}
                </em>
                ) but no study material has been authored for it in this app
                yet.
              </>
            )}
            {topic.sharedWith &&
              topic.sharedWith.length > 0 &&
              !hasResources && (
                <>
                  {" "}
                  Some <strong>other specializations</strong> may already cover
                  this material — check the related-specs badges above.
                </>
              )}
          </div>
        );
      ...
      {topic.refs.map(([cid, tid, sid], i) => {
        ...
        return (
          <article
            ...
            data-collapsed={subCollapsed}
            data-tier={subTier ? subTier.kind : undefined}
            style={subTier ? { "--tier-h": subTier.hue } : undefined}
          >
            <header className="subtopic-head">
              <div className="subtopic-title-row">
                <ChevronToggle
                  collapsed={subCollapsed}
                  onClick={() => toggleCollapsed(subKey, subDefaultCollapsed)}
                  label={sub.title}
                />
                <button
                  className="subtopic-check"
              ...
              <button
                className="subtopic-anchor"
                style={{ opacity: 1 }}
                onClick={() => navigate(`/c/${cid}/${tid}/${sid}`)}
              >
                <svg
                  width="12"
                  height="12"
        ...
        <div className="overlay" onClick={() => setShowJump(false)}>
          <div className="sheet" onClick={(e) => e.stopPropagation()}>
            <div className="sheet-head">
              ...
              <button className="icon-btn" onClick={() => setShowJump(false)}>
                ×
              </button>
            </div>
            <div className="exam-topiclist">
              {topics.map((t) => (
                <button
                  key={t.id}
                  className="exam-topic-card"
                  data-active={t.id === topicId}
                  onClick={() => {
                    setShowJump(false);
                    navigate(`/x/${specId}/${t.id}`);
                  }}
                >
                  <span className="exam-topic-n">
                    {String(t.n).padStart(2, "0")}
